...
export default function ResendVerificationPage(){
  ...
  async function submit(event:FormEvent<HTMLFormElement>){event.preventDefault();const email=String(new FormData(event.currentTarget).get('email')??'').trim();setBusy(true);setError('');try{await resendVerification(email);setMessage(t('recovery.resendSuccess'));event.currentTarget.reset();}catch(problem){setError(problem instanceof Error?problem.message:t('auth.failed'));}finally{setBusy(false);}}
