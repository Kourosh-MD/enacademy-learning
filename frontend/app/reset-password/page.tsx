...
function ResetPasswordForm(){
  ...
  async function submit(event:FormEvent<HTMLFormElement>){event.preventDefault();const form=event.currentTarget;const data=new FormData(form);const password=String(data.get('password')??'');const confirmation=String(data.get('confirmation')??'');setError('');if(!/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d).{10,72}$/.test(password)){setError(t('auth.passwordInvalid'));return;}if(password!==confirmation){setError(t('recovery.passwordMismatch'));return;}setBusy(true);try{await resetPassword(token,password);form.reset();setDone(true);}catch(problem){setError(problem instanceof Error?problem.message:t('auth.failed'));}finally{setBusy(false);}}
