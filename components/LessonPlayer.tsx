...
export function LessonPlayer({ lesson, savedWords }: { lesson: Lesson; savedWords: string[] }) {
  ...
  async function toggleWord(word: string) {
    ...
    const next = new Set(saved); shouldSave ? next.add(word) : next.delete(word); setSaved(next);
    await fetch('/api/words', { method:'POST', headers:{'content-type':'application/json'}, body:JSON.stringify({word,save:shouldSave}) });
  }
  ...
  async function finishLesson() {
    ...
    const response = await fetch('/api/progress', { method:'POST', headers:{'content-type':'application/json'}, body:JSON.stringify({lessonId:lesson.id,score}) });
    setCompleteState(response.ok ? 'saved' : 'idle');
  }
